  ...
  return (
    <tr className={s.collection_table_row}>
      <td className={s.collection_logo}>
        <img src={item.image} alt="recently1" className={s.collection_logo_img}/>
      ...
      <td>
        <img src="img/evirium.svg" alt="evirium-logo" className={s.collection_efirium_logo}/>
        <span className={s.collection_efirium_logo_span}>{item.volume}</span>
      </td>
      <td className={s.collection_item_list_item_procent} style={{ color: item.procent > 0 ? 'green' : 'red' }}>
